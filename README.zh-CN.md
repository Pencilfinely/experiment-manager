...
**0.3.0-rc.1 是桌面版预览。** 管理端与算力端独立安装。同一台电脑要同时管理实验并提供显卡，就安装两个版本。
这次提供 Windows 安装程序、应用入口和后台运行；尚未提供签名 MSI、自动更新或无人值守操作系统安装。

## 下载与安装

从 [GitHub Releases](https://github.com/Pencilfinely/experiment-manager/releases) 下载应用安装包或完整 ZIP。GitHub 自动生成的 Source code 不是安装包。
...
运行 **Experiment Center**，打开实验台窗口。侧栏包含总览、实验、算力、算法项目和设置。
...
| 一张卡同时跑多个独立实验 | 支持，需任务允许共享、单卡并发上限允许且预算足够 |
| 多卡/多机合跑一个实验 | 尚未实现，需算法与调度器共同支持 |
| 自动测量并选择最快分配 | 尚未实现 |

空闲显存不等于空闲算力。软件不会偷偷修改 batch、精度或学习率来让任务放得下。
主控暂时离线时，节点可继续已分配、已缓存的任务，恢复连接后补传；不会擅自把失联任务重复派到另一台机器。

## 已有部署与日常维护

升级前先等任务与回传完成，在客户端停止旧管理服务/算力代理，再从托盘退出旧客户端，最后安装同角色新版。安装器检测到同角色旧客户端仍运行时会要求先退出，不提供运行中的热替换。选择原管理数据目录、原 Ubuntu 和原节点配置，保留身份与实验记录。
不要让新旧两个代理同时使用同一个节点目录。完整操作、备份、后台停止和排错见 [日常使用说明](docs/OPERATIONS.zh-CN.md)。
仅在本机或可信私网使用本预览版；它不是直接开放公网的多租户服务。
