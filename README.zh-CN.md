...
**当前版本为 0.4.3。** 管理端与算力端独立安装。同一台电脑要同时管理实验并提供显卡，就安装两个版本。
包含实验矩阵、自动/半自动/手动算力分配、表单导入与可选 AI 辅助、受管理项目清理和 Markdown 结果导出。新实验默认允许共享 GPU，可在算力页调整已有节点的并发和资源预算，单次实验及矩阵也能直接修改预算与共享方式。详见[发布说明](docs/RELEASE-0.4.3.md)。更新由你主动操作。
