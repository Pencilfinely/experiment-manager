...
**0.4.2 已作为正式版本发布。** 管理端与算力端独立安装。同一台电脑要同时管理实验并提供显卡，就安装两个版本。
包含实验矩阵、自动/半自动/手动算力分配、表单导入与可选 AI 辅助、受管理项目清理和 Markdown 结果导出。管理端更新不再仅因空闲节点离线或心跳过期而被阻挡，并包含停止状态下的后台升级与版本显示修复。详见[发布说明](docs/RELEASE-0.4.2.md)。更新由你主动操作。
