...
本说明适用于 **0.3.0-rc.4 桌面预览版**。日常操作从 Experiment Center 和 Experiment Worker 进入；旧版终端入口保留作兼容和诊断用途。
...
**多机环境先更新 Worker，恢复在线并完成同步，再更新 Center。** Center 需要每个算力端提供最新空闲确认；算力端离线，或旧版代理无法提供确认时，会阻止应用内安装。先更新对应 Worker 并恢复连接，再重试 Center 更新。
...
| Center 无法确认算力端空闲 | 先更新对应 Worker，恢复在线并完成同步，再重试 Center；离线或旧版代理状态未知时会阻止安装 |
