...
3. 运行对应角色的 rc.4 `Setup.exe`，保留原管理数据目录、Ubuntu 和算力节点配置。不要把安装目录改成实验数据目录。
...
- Windows 管理端：`ExperimentManager-0.3.0-rc.4-windows-controller-x64-Setup.exe`
- Windows 算力端：`ExperimentManager-0.3.0-rc.4-windows-worker-x64-Setup.exe`
- Ubuntu 算力端：`ExperimentManager-0.3.0-rc.4-ubuntu-worker-x64.zip`
...
If the backend still reports rc.1, stop it using the old client's service controls, exit the tray client, and run the rc.4 installer directly once. Keep the existing data directory, Ubuntu distribution and worker configuration. Update workers before the controller.
