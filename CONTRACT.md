...
Public `/mobile/` (also `/mobile`) serves the shared Android / HarmonyOS UI; `/mobile-access` serves the administrator's mobile-device authorization page. Static mobile files are explicitly allowlisted and carry a same-origin CSP. No CORS or offline service worker is introduced.

Admin-only GET `/api/mobile/devices` lists metadata, POST `/api/mobile/devices {name,permission:monitor|control,days?:1..90}` issues one random bearer token (default 30 days), and POST `/api/mobile/revoke {id}` revokes it. Mobile credentials are stored only as SHA-256 in SQLite; they never authenticate to admin or worker endpoints. Existing admin and node credentials do not authenticate to mobile endpoints.

Both mobile permissions may GET `/api/mobile/state` (project/source/template-free projection, nodes, server time and session scope) and `/api/mobile/job?id=...` (detail, logs, metrics history and archive metadata). Control permission additionally permits POST `/api/mobile/action {job_id,action:stop|cancel|resume,expected_command_id}` and `/api/mobile/node-mode {node_id,mode:run|drain,expected_mode}`. Commands reuse the existing hub methods, retaining native-resume restrictions and durable acknowledgments. Compare the expected command ID or node mode inside the mutation transaction; stale submissions return 409. Revocation and mobile authorization/dispatch are serialized under the hub lock. Other mobile paths are denied; no raw admin credential is exported.

The mobile page polls every five seconds while visible, disables controls on failure or after fifteen seconds without a snapshot, refreshes on foregrounding, and does not queue or automatically retry commands. Saving a mobile credential in localStorage requires the user's checkbox; otherwise it remains in sessionStorage. Logout clears both. Applications are thin platform containers, with no native JavaScript bridge. Android and HarmonyOS build/signing and real-device acceptance remain separate from web/API tests; see `mobile/README.md`.
